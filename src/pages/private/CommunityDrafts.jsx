...
function CommunityDrafts() {
  ...
  const load = async () => {
    ...
    setError('')
    try {
      const result = await getPosts({ scope: 'drafts' })
  ...
  const handleUpdated = async (updated) => {
    const saved = await updatePost(updated.$id, {
      title: updated.title,
    ...
    if (saved.status === 'published') {
      setPosts((prev) => prev.filter((item) => item.$id !== saved.$id))
    } else {
      setPosts((prev) =>
      ...
      <main className="mx-auto max-w-7xl px-6 py-10">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black tracking-tight">My drafts</h1>
            <p className="mt-2 max-w-2xl text-lg text-ink-muted">
              Unpublished posts only you can see. Publish them when they are ready.
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="mt-8 rounded-xl border border-accent-yellow bg-cream px-6 py-12 text-center">
            <Icon name="file-text" size={40} className="mx-auto text-brand-deep" />
            <p className="mt-4 text-2xl font-black tracking-tight">No drafts</p>
            <p className="mt-2 mx-auto max-w-md text-sm text-ink-muted">
              Save a post as a draft and it will appear here for you to finish later.
            </p>
