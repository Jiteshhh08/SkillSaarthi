...
### 6.6 Icons, brand marks & decorative shapes

All icons are the professional Lucide line-icon set (`lucide-react`), never emoji;
wired through `src/components/common/Icon.jsx` (semantic name → component). The one
exception is the **GitHub** mark, a real brand SVG (`src/components/common/Icon.jsx`,
`GithubIcon`) so product pages always show the authentic logo. Icons render at
`currentColor`, sized contextually (16px inline pills, 22–26px tiles, 30–40px heroes).

Decorative vector shapes — low-opacity circles in mint/amber, exactly the pattern in the
Dashboard hero (e.g. a 176px `bg-brand/10` circle top-right and a 80px `bg-accent-yellow/20`
circle) — are generated by `src/components/common/DecorativeShapes.jsx`:
- `variant="default"` — hero-size pair (`h-44 w-44 bg-brand/10`, `h-20 w-20 bg-accent-yellow/20`).
- `variant="band"` — large side blobs (`h-40 w-40 bg-brand/10`, `h-48 w-48 bg-accent-yellow/15`)
  for full-width bands (Home heroes, WhatWeDo "How it works" band, promo band).
- `variant="card"` — subtle corner circles (`h-24 w-24 bg-brand/5`, `h-12 w-12 bg-accent-yellow/10`)
  for cards and tiles.

Hosts set `relative overflow-hidden`; content sits above via `position: relative`.
...
- **Hero banner:** rounded-2xl gradient band (`#e3f9f1 → #ffffff → #f7f8fa`) with eyebrow,
  ...
  for streak / education level / assessment score (Lucide flame, graduation-cap, chart marks).
- **Stat tiles:** 4-across (2-across on mobile) — Profile setup %, skills added, interests,
  ...
  card-layer decorative shapes sit behind the value.
...
- **Career tools grid:** 8 tool cards (4-across ≥1200px), each a white card with a 48px rounded
  Lucide icon tile (mint/blue/amber/purple/deep-green tints) and a `card` decorative shape;
  16px/700 title, muted description, and a deep-green "→" link that underlines on hover;
  cards lift on hover.
