...
> No secrets in Git — all `.env` gitignored, production values in the Vercel/Render/SnapDeploy dashboards. Frontend `VITE_*` are build-time (`vite.config.js`); backend `server/.env` via `environment.js` (never `VITE_`); AI `ai-service/.env` via `ai/client.py`; scripts via `scripts/.env.setup` (setup/seed/importer). Free plan reuses `resumes` bucket for avatars; paid plan uses `avatars`. Email OFF by default (`isEmailConfigured()` false → `_dev_otp` mock); enable via SendGrid HTTPS (Render blocks SMTP). `VITE_API_BASE_URL` change requires Vercel redeploy + hard-refresh.
>
...
| AI service (Python + FastAPI) | SnapDeploy (Docker) | `https://skillsaarthi-ai-ebeba.containers.snapdeploy.app` | `/health` |
...
(cloud.appwrite.io)                 (Render — Node.js backend)
                                                 │
                                                 ▼
                                          https://skillsaarthi-ai-ebeba.containers.snapdeploy.app
                                          (SnapDeploy — Python FastAPI AI service, Docker)
```
...
| SnapDeploy | AI service (Python, Docker) | Dockerfile in `ai-service/`, port 7860, auto-sleep + cron keep-warm |
...
AI_SERVICE_URL=https://YOUR-AI.snapdeploy.app        # e.g. https://skillsaarthi-ai-ebeba.containers.snapdeploy.app (no trailing slash)
...
### AI service (SnapDeploy env vars)

Deployed as a Docker container from the same repo: **SnapDeploy → Deploy from GitHub → Root Directory `ai-service` → Dockerfile Path `Dockerfile` → Port `7860`**.
...
`PORT` is injected by SnapDeploy; the container listens on `${PORT:-7860}`.
...
> Edit env vars in SnapDeploy → rolling restart, no code change or redeploy.
...
### SnapDeploy notes — free tier, sleep & keep-warm

- Free tier: 4 containers, 512 MB RAM, 0.25 vCPU, 10 deploys/day (5 per 12h), no card.
- Containers auto-sleep after 15 min idle and wake in ~60 s. Keep the AI service warm with a
  cron-job.org job hitting `/health` every 5 minutes (timeout 90 s) so the backend's 15 s AI
  timeout never races a cold wake.
- Env changes are applied with a zero-downtime rolling restart (~2–3 min); `PORT` is managed and locked.
...
- **Health checks:** `https://YOUR-BACKEND.onrender.com/health` (`server/src/app.js`) and `https://YOUR-AI.snapdeploy.app/health` (`ai-service/app/main.py`) must return `{"status":"ok",...}`. Point cron-job.org (every 5 min) at `/health` (not `/`) to keep both free tiers awake.
...
## 47.5 Render / SnapDeploy Service Settings

| Setting | Backend (`skillsaarthi-node`, Render) | AI service (`skillsaarthi-ai-ebeba`, SnapDeploy) |
| --- | --- | --- |
...
| Port | `$PORT` (injected; 5000 local) | `7860` (SnapDeploy-injected `PORT` preferred) |
...
1. Deploy the **AI service** first (SnapDeploy → root `ai-service`, Dockerfile `Dockerfile`, port 7860); copy `https://skillsaarthi-ai-ebeba.containers.snapdeploy.app`.
...
* Render free tier grants **750 instance-hours/month** per workspace — keeping two services awake (two cron jobs) blew the budget and suspended all free services. Only the Node backend stays on Render now (~744 h/month max); the AI service runs on SnapDeploy. Keep both warm with cron-job.org (every 5 minutes): `skillsaarthi-node.onrender.com/health` and `skillsaarthi-ai-ebeba.containers.snapdeploy.app/health` (SnapDeploy sleeps after 15 min idle, ~60 s wake).
