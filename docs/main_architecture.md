...
> No secrets in Git — all `.env` gitignored, production values in the Vercel/Render dashboards (+ `fly secrets` for the AI service). Frontend `VITE_*` are build-time (`vite.config.js`); backend `server/.env` via `environment.js` (never `VITE_`); AI `ai-service/.env` via `ai/client.py`; scripts via `scripts/.env.setup` (setup/seed/importer). Free plan reuses `resumes` bucket for avatars; paid plan uses `avatars`. Email OFF by default (`isEmailConfigured()` false → `_dev_otp` mock); enable via SendGrid HTTPS (Render blocks SMTP). `VITE_API_BASE_URL` change requires Vercel redeploy + hard-refresh.
>
...
| AI service (Python + FastAPI) | Fly.io (Docker) | `https://skillsaarthi-ai.fly.dev` | `/health` |
...
(cloud.appwrite.io)                 (Render — Node.js backend)
                                                 │
                                                 ▼
                                           https://skillsaarthi-ai.fly.dev
                                           (Fly.io — Python FastAPI AI service, Docker)
```
...
| Fly.io | AI service (Python, Docker) | `ai-service/fly.toml` + `Dockerfile`, port 7860, `min_machines_running = 1` (always on) |
...
AI_SERVICE_URL=https://YOUR-AI.fly.dev                # e.g. https://skillsaarthi-ai.fly.dev (no trailing slash)
...
### AI service (Fly.io env vars / secrets)

Deployed as a Docker container from the same repo: **`fly deploy` from `ai-service/`** (config in `ai-service/fly.toml`, Dockerfile build, internal port `7860`, region `bom`).
...
`PORT` is injected by Fly.io; the container listens on `${PORT:-7860}`. Set secrets with `fly secrets set AI_BASE_URL=... AI_MODEL=... AI_KEY=...`.
...
> Edit secrets via `fly secrets set ...` → machine restart, no code change or redeploy.
...
### Fly.io notes — always-on machine

- One shared-cpu-1x / 512 MB machine, `min_machines_running = 1` (`ai-service/fly.toml`) — the AI
  service never sleeps, so the backend's AI timeouts never race a cold wake and no cron keep-warm
  job is needed for it.
- To save resources, switch `auto_stop_machines` to `"suspend"` in `fly.toml` (wakes on request).
- Secrets are managed with `fly secrets set ...`; `PORT` is injected by Fly.io.
...
- **Health checks:** `https://YOUR-BACKEND.onrender.com/health` (`server/src/app.js`) and `https://YOUR-AI.fly.dev/health` (`ai-service/app/main.py`) must return `{"status":"ok",...}`. Point cron-job.org (every 5 min) at the backend `/health` (not `/`) to keep the Render free tier awake; the Fly.io AI service stays on by itself.
...
## 47.5 Render / Fly.io Service Settings

| Setting | Backend (`skillsaarthi-node`, Render) | AI service (`skillsaarthi-ai`, Fly.io) |
| --- | --- | --- |
| Environment | Node | Docker (`ai-service/Dockerfile` + `fly.toml`) |
| Root directory | `server` | `ai-service` |
| Dockerfile path | — | `Dockerfile` (via `[build]` in `fly.toml`) |
...
| Port | `$PORT` (injected; 5000 local) | `7860` (Fly.io-injected `PORT` preferred) |
...
1. Deploy the **AI service** first (`fly deploy` from `ai-service/`, port 7860); copy `https://skillsaarthi-ai.fly.dev`.
...
* Render free tier grants **750 instance-hours/month** per workspace — keeping two services awake (two cron jobs) blew the budget and suspended all free services. Only the Node backend stays on Render now (~744 h/month max); the AI service runs on Fly.io with `min_machines_running = 1`. Keep the backend warm with cron-job.org (every 5 minutes): `skillsaarthi-node.onrender.com/health`.
