...
> Four services: Frontend (Vercel static, `https://skillsaarthi.vercel.app`), Backend Node (Render `https://skillsaarthi-node.onrender.com` `/api/health`), AI Python (Render `https://skillsaarthi-f14x.onrender.com` `/health`), Data (Appwrite Cloud). Set `VITE_API_BASE_URL` to Render backend (must redeploy), backend `APPWRITE_*` + `AI_SERVICE_URL` + `GITHUB_TOKEN`/`ADMIN_EMAILS`/`FRONTEND_URL`/`SENDGRID_API_KEY` + `SENDGRID_SENDER` (verified Single Sender, SendGrid HTTPS at `email.service.js:77`) on Render (PYTHON_VERSION=3.12.10 for AI), add Vercel URL to Appwrite Platforms, run `setup:appwrite` + `seed:catalog`, keep free tier awake via cron-job.org every 5min on `/health`.
>
